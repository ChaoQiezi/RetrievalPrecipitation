...
from utils.utils import create_xy
...
# 数据集划分和标准化
split_time = datetime(2020, 7, 31)  # 划分时间节点, 5~7月为训练集, 8月为验证集, 约为3:1
...
train_x, train_y, train_ix = create_xy(train_ds, window_size=10, step_size=3)
test_x, test_y, test_ix = create_xy(test_ds, window_size=10, step_size=3)
# 输出为HDF5文件
